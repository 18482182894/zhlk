    ...
    <div class="menuManage operationLog ">
      ...
      <table class="customTable">
        ...
        <tr v-for="(item,i) in listData">
          ...
          <td v-text="item.operateDate.slice(0, 10)"></td>
          <td v-text="draftStatusMap[item.draftStatus]"></td>
          <td v-text="auditStateMap[item.auditState]"></td>
          <td>
            <div class="table-btn btn-yellow"
                 @click="jumpDetail(item.kichoDivergenceGasJobId)">详情
            </div>
            <div class="table-btn btn-submit"
                 @click="submitAudit(item.kichoDivergenceGasJobId)" v-if="item.draftStatus==0">提交审核
            </div>
            <div class="table-btn btn-green"
                 @click="jumpEdit(item.kichoDivergenceGasJobId)" v-if="item.draftStatus==0">编辑
            </div>
            <div class="table-btn btn-red"
                 @click="delData(item.kichoDivergenceGasJobId)" v-if="item.draftStatus==0">删除
            </div>
            <div class="table-btn btn-again"
                 @click="submitAudit(item.kichoDivergenceGasJobId)" v-if="item.auditState==0">再次申请
            </div>
          </td>
        </tr>
        </tbody>
...
<script>
  import {fetchExhaustingQiList, delExhaustingQi, applyExhaustingQi} from "@/api/intelligentWarehous/atmosphere";
  ...
  export default {
    ...
    methods: {
      ...
      getList(pageNo) {//获取列表
        ...
        layer.load(2);
        fetchExhaustingQiList(vm.postData)
          .then(ret => {
      ...
      submitAudit(id) {
        layer.confirm(
          ...
          () => {
            layer.load(2);
            applyExhaustingQi({kichoDivergenceGasJobId: id})
              .then((ret) => {
                layer.closeAll("loading");
                if (ret.code == 200) {
      ...
      delData(id) {
        layer.confirm(
          ...
          () => {
            layer.load(2);
            delExhaustingQi({kichoDivergenceGasJobIdList: id})
              .then((ret) => {
                layer.closeAll("loading");
                if (ret.code == 200) {
                  this.detail = ret.data;
