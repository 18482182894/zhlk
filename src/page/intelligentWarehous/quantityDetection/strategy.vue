  ...
  <div class="content">
    ...
    <StrategyDetailPopup
      ...
      <template slot-scope="scope">
        <div v-if="scope.dataPo">
          <div class="extra">
            <div class="extra-label">粮食平整度预警阈值：</div>
            <div class="extra-area">{{scope.dataPo.grainSurfacePlanenessEarlyWarningThresholdJudge === '不限制' ? '不限制' : scope.dataPo.grainSurfacePlanenessEarlyWarningThresholdJudge + scope.dataPo.grainSurfacePlanenessEarlyWarningThresholdValue}}</div>
          </div>
          <div class="extra">
            <div class="extra-label">账实差异率阀值(%)：</div>
            <div class="extra-area">{{scope.dataPo.chomensaritsuThresholdJudge === '不限制' ? '不限制' : scope.dataPo.chomensaritsuThresholdJudge + scope.dataPo.chomensaritsuThresholdValue}}</div>
          </div>
          <div class="extra">
            <div class="extra-label">待补仓数量预警阈值：</div>
            <div class="extra-area">{{scope.dataPo.hokurasuryoJizenkekaiThresholdJudge === '不限制' ? '不限制' : scope.dataPo.hokurasuryoJizenkekaiThresholdJudge + scope.dataPo.hokurasuryoJizenkekaiThresholdValue}}</div>
          </div>
        </div>
      </template>
    </StrategyDetailPopup>
...
<script>
  ...
  import {addGrainQuantityStrategy, editGrainQuantityStrategy, fetchGrainQuantityStrategy, fetchGrainQuantityStrategyList} from '@/api/intelligentWarehous/strategy';
  import StrategyPopup from '../GrainQuantityStrategyPopup';
  ...
  export default {
    ...
    methods: {
      getList: function (pageNo) {
        ...
        layer.load(2);
        fetchGrainQuantityStrategyList(vm.postData)
          .then(ret => {
      ...
      getStrategyDetail() {
        return fetchGrainQuantityStrategy({warehouseGbCode: this.currentWarehouseGbCode})
      },
      submit(data) {
        layer.load(2);
        const api = this.isAddStrategy ? addGrainQuantityStrategy: editGrainQuantityStrategy;
        api(data)
          .then(ret => {
            layer.closeAll("loading");
            if (ret.code == 200) {
