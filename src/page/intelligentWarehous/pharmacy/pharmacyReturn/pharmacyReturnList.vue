    ...
    <div class="menuManage operationLog priceManage">
      <div class="lookup">
        ...
        <div style="display: inline-block">
          <div class="btn btn-add" style="margin-top: -8%" @click="getList(0);">
      ...
      <table class="customTable">
        ...
        <tr>
          ...
          <th style="width:10%;">归还数量</th>
          <th style="width:10%;">归还人</th>
          <th style="width:10%;">签收人</th>
        ...
        <tr v-for="(item,i) in listData">
          ...
          <td>{{item.giveBackNum}}</td>
          <td>{{item.giveBackPerson}}</td>
          <td>{{item.signPerson}}</td>
          <td>{{item.giveBackDatetime}}</td>
          <td>
            <div class="table-btn btn-yellow" @click="jumpDetail(item.giveBackId)">详情</div>
          </td>
        </tr>
        </tbody>
      </table>
...
<script>
  import {fetchGiveBackList} from "@/api/intelligentWarehous/pharmacy";
