    ...
    <div class="menuManage operationLog ">
      ...
      <table class="customTable">
        ...
        <tr>
          ...
          <th style="width:5%;">调试责任人</th>
          <th style="width:10%;">调式记录</th>
        ...
        <tr v-for="(item,i) in listData">
          <td><input type="checkbox" :value="item.equipmentDebugId" v-model="delList.equipmentDebugIdList" @change="clickChange();"></td>
          <td v-if="sortOrder=='0'" v-text="i+1"></td>
          <td v-else v-text="listData.length-i"></td>
          <td v-text="item.equipmentName"></td>
          <td v-text="item.equipmentType=='0'?'计量（检验）设备':'仓储设备'" ></td>
          <td v-text="item.debugAdminName"></td>
          <!--<td v-text="item.debugFee"></td>-->
          <td v-text="item.debugDetail"></td>
          <td v-text="item.debugTime"></td>
          ...
          <td>
            <div class="table-btn btn-yellow" @click="jumpDetail(item.equipmentDebugId);">详情</div>
            ...
            <div class="table-btn btn-submit" @click="submitAudit(item.equipmentDebugId);" v-if="item.draftStatus==0">提交审核</div>
            <!-- <div class="table-btn btn-again" v-if="item.status==0" @click="jumpEdit(item,'edit');">再次申请</div> -->
            <div class="table-btn btn-red" @click="delData(item.equipmentDebugId);" v-if="item.draftStatus==0">删除</div>
          </td>
        </tr>
        </tbody>
      </table>
...
<script>
  import {getDebugList , deleDebug,submitDebugByCheck} from "../../../api/devicesManage/debug";
  ...
  export default {
      ...
      methods: {
        ...
        delData: function(id) {
          ...
          var vm = this;
          if (id) {
            vm.delList.equipmentDebugIdList = [];
            vm.delList.equipmentDebugIdList.push(id);
          } else {
            if (vm.delList.equipmentDebugIdList.length == []) {
              layer.msg("请至少选择一条数据！");
              return;
            }
          }
          layer.confirm('确定删除？', {
            ...
            layer.load(2);
            var delStr = vm.delList.equipmentDebugIdList.join(",");
            deleDebug({ equipmentDebugIdList: delStr })
              .then(ret => {
                layer.closeAll("loading");
                if (ret.code == 200) {
                  layer.msg("删除成功！");
        ...
        checkAll: function() {
          ...
          if (vm.allChecked) {
            for (var i = 0; i < vm.listData.length; i++) {
              vm.delList.equipmentDebugIdList.push(vm.listData[i].equipmentDebugId);
            }
          } else {
            vm.delList.equipmentDebugIdList = [];
          }
        ...
        clickChange: function() {
          ...
          var vm = this;
          if (vm.delList.equipmentDebugIdList.length == vm.listData.length) {
            vm.allChecked = true;
          } else {
            vm.allChecked = false;
          }
    ...
    detailBack:function () {
        ...
        submitAudit:function (id) {//提交审核
            var vm=this;
            submitDebugByCheck({ equipmentDebugId: id }).then(ret => {
                layer.closeAll("loading");
                if (ret.code == 200) {
                  layer.msg("提交成功！");
                  vm.getList(1);
