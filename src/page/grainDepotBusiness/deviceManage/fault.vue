    ...
    <div class="menuManage operationLog priceManage">
      ...
      <table class="customTable">
        ...
        <tr>
          ...
          <th style="width:5%;">故障责任人</th>
          <th style="width:10%;">故障备注</th>
        ...
        <tr v-for="(item,i) in listData">
          <td><input type="checkbox" :value="item.equipmentFailureId" v-model="delList.equipmentFailureIdList" @change="clickChange();"></td>
          <td v-if="sortOrder=='0'" v-text="i+1"></td>
          <td v-else v-text="listData.length-i"></td>
          <td v-text="item.equipmentName"></td>
          <td v-text="item.equipmentType=='0'?'计量(检验)设备':'仓储设备'" ></td>
          <td v-text="item.failureAdminName"></td>
          <td v-text="item.memo"></td>
          <td v-text="item.findTime"></td>
          ...
          <td>
            <div class="table-btn btn-yellow" @click="jumpDetail(item.equipmentFailureId);">详情</div>
            <div class="table-btn btn-green" @click="jumpEdit(item.equipmentFailureId,'edit');" v-if="item.draftStatus==0">编辑</div>
            ...
            <div class="table-btn btn-submit" @click="submitAudit(item.equipmentFailureId);" v-if="item.draftStatus==0">提交审核</div>
            <!--<div class="table-btn btn-again" v-if="item.status==0" @click="jumpEdit(item.customerId,'add');">再次申请</div>-->
            <div class="table-btn btn-red" @click="delData(item.equipmentFailureId);" v-if="item.draftStatus==0">删除</div>
          </td>
        </tr>
        </tbody>
      </table>
...
<script>
  import { getEquipmentFailureList , deleEquipmentFailure ,submitEquipmentFailureByCheck} from "../../../api/devicesManage/equipmentFailure";
  ...
  export default {
    ...
    methods: {
      ...
      getList: function(pageNo) {//获取列表
        ...
        layer.load(2);
        getEquipmentFailureList(vm.postData)
          .then(ret => {
      ...
      delData: function(id) {
        ...
        var vm = this;
        if (id) {
          vm.delList.equipmentFailureIdList = [];
          vm.delList.equipmentFailureIdList.push(id);
        } else {
        ...
        layer.confirm('确定删除？', {
          ...
          layer.load(2);
          var delStr = vm.delList.equipmentFailureIdList.join(",");
          deleEquipmentFailure({ equipmentFailureIdList: delStr })
            .then(ret => {
              layer.closeAll("loading");
              if (ret.code == 200) {
                layer.msg("删除成功！");
      ...
      checkAll: function() {
        ...
        if (vm.allChecked) {
          for (var i = 0; i < vm.listData.length; i++) {
            vm.delList.equipmentFailureIdList.push(vm.listData[i].equipmentFailureId);
          }
        } else {
          vm.delList.equipmentFailureIdList = [];
        }
      ...
      clickChange: function() {
        ...
        var vm = this;
        if (vm.delList.equipmentFailureIdList.length == vm.listData.length) {
          vm.allChecked = true;
        } else {
          vm.allChecked = false;
        }
    ...
    detailBack:function () {
      ...
      submitAudit: function(id) {//提交审核
            var vm=this;
            submitEquipmentFailureByCheck({ equipmentFailureId: id }).then(ret => {
                layer.closeAll("loading");
                if (ret.code == 200) {
                  layer.msg("提交成功！");
                  vm.getList(1);
