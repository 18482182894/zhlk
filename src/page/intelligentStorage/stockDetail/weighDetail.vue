      ...
      <div class="content" id="div_print" style="display: table;">
        <div class="wrapepr" v-if="isShow ==1">
          ...
          <table>
            ...
            <tr>
              <td>仓   号</td>
              <td colspan="2">{{weighInfo.storehouseName}}</td>
              <td>扣   水</td>
        ...
        <div class="wrapepr" v-if="isShow == 2" style="display: table;">
          <p class="ZY_title">粮食转运收件单</p>
          <p style="text-align: left;margin-left: 5%;">
            <span>结算单位：三台琴泉粮站</span>
            <span ref="ZY_time" style="width: 340px;"><!--{{year_}}年{{month_}}月{{day_}}日-->
...
<script>
import { myPrint,changeMoneyToChinese,changeNumberToChinese,getNowFormatDate } from "@/utils/public";
import { getWeightResult } from "@/api/intelligentStorage/public";
export default {
  name: "document",
  data() {
    return {
  ...
  methods: {
    ...
    getWeightById: function(){
        ...
        getWeightResult({registorNo: this.id_}).then( res => {
            if(res.code == 200){
              //console.log(res);
