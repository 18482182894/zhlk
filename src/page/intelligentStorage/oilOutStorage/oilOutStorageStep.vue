...
<script>
  ...
  import { oilRegisterFlowList, oilMeteringOutFlowList, oilNoValueBinOutList, oilAlreadyValueBinOutList, oilSettlementList, processPinCardFlowList, oilUnsoldCardList ,pinCard} from "@/api/intelligentStorage/public";
    ...
    methods: {
      ...
      getList: function(pageNo,type) {//获取列表
        ...
        switch(vm.guideStep)
        {
          ...
          case 4:
            ...
            oilMeteringOutFlowList(vm.meteringPostData)
              .then(ret => {
                layer.closeAll("loading");
                if (ret.code == 200) {
          ...
          function noValueBin() {
            oilNoValueBinOutList(vm.valueBinPostData)
              .then(ret => {
                if (type) {
                  layer.closeAll("loading");
                }else{
          ...
          function alreadyValueBin() {
            oilAlreadyValueBinOutList(vm.valueBinPostData)
              .then(ret => {
                if (type) {
                  layer.closeAll("loading");
                }else{
      ...
      jumpEdit: function(id) {//跳转到编辑或新增页面
        ...
        switch(vm.guideStep){
          case 1:
            condition=vm.registerPostData;
            url='oilRegisterOutEdit';
            break;
          case 4:
            condition=vm.meteringPostData;
            url='oilMeteringOutEdit';
            break;
          case 5:
            condition=vm.valueBinPostData;
            url='oilValueBinOutEdit';
            break;
          case 6:
            condition=vm.settlementPostData;
            url='oilSettleOutEdit';
            break;
          case 7:
            condition=vm.unsoldCardPostData;
            url='oilPinOutCard';
