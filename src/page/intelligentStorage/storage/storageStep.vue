      ...
      <table class="customTable">
        ...
        <tbody>
            <tr v-for="(item,i) in listData">
                ...
                <td v-text="item.plateNumber"></td>
                <td v-text="item.grainVariety"></td>
                <td v-text="item.storehouseNumber"></td>
                <td>
                      <div class="table-btn btn-yellow" @click="jumpDetail(item.registorNo);">详情</div>
      ...
      <table class="customTable">
        <thead>
          <tr>
            <th style="width:5%;"><input type="checkbox" ></th>
            <th style="width:5%;">序号</th>
            <th style="width:7.5%;">入库单号</th>
            <th style="width:7.5%;">入库类型</th>
...
import { registerFlowList,samplingFlowList ,checkoutFlowList, meteringFlowList, noValueBinList, alreadyValueBinList, settlementList, processPinCardFlowList,unsoldCardList ,pinCard} from "@/api/intelligentStorage/public";
    ...
    getList: function(pageNo,type) {//获取列表
      ...
      switch(vm.guideStep)
      {
          ...
          case 2:
            ...
            samplingFlowList(vm.samplingPostData)
              .then(ret => {
                layer.closeAll("loading");
                if (ret.code == 200) {
          ...
          case 3:
            ...
            checkoutFlowList(vm.checkPostData)
              .then(ret => {
                layer.closeAll("loading");
                if (ret.code == 200) {
                  vm.listData = ret.data.dataList;
          ...
          case 4:
            ...
            meteringFlowList(vm.meteringPostData)
              .then(ret => {
                layer.closeAll("loading");
                if (ret.code == 200) {
          ...
          case 5:
            ...
            function noValueBin() {
              noValueBinList(vm.valueBinPostData)
                .then(ret => {
                  if (type) {
                    layer.closeAll("loading");
                  }else{
            ...
            function alreadyValueBin() {
              alreadyValueBinList(vm.valueBinPostData)
                .then(ret => {
                  if (type) {
                    layer.closeAll("loading");
                  }else{
