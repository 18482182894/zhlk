          ...
          if (ret.code == 200) {
            vm.detailData = ret.data.ondutyDetail;
             ...
             grainDetail({grainVarietyId:ret.data.ondutyDetail.grainVariety}).then(ret=>{
              ...
              warehouseDetail({storehouseGbCode:ret.data.ondutyDetail.storehouseGbCode}).then(ret=>{
                  if (ret.code==200) {
                      vm.detailData.storehouseGbCode=ret.data.dataMap.storehouseName;
                  }else{
                      layer.alert(ret.data, { closeBtn: 0 });
                  }
              ...
              granaryDetail({warehouseGbCode:ret.data.ondutyDetail.warehouseGbCode})
                .then(ret => {
                  layer.closeAll("loading");
                  if (ret.code == 200) {
                    vm.detailData.warehouseGbCode = ret.data.dataMap.warehouseName;
              ...
              goodsDetail({cargoPosition:ret.data.ondutyDetail.cargoPosition})
                .then(ret => {
                  layer.closeAll("loading");
                  if (ret.code == 200) {
                    vm.detailData.cargoPosition = ret.data.cargoDetail.cargoName;
