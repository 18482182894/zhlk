                    ...
                    <div class="userManage-detail userManage-edit">
                        <div>
                            ...
                            <span>
                                ...
                                <input type="text" class="input-text" v-model="submitData.grossWeight">
                            </span>
                            <span>
                                皮重(千克)：
                                <!-- <input type="number" class="input-text" readonly="readonly"  v-model="submitData.tareWeight"> -->
                                <input type="number" class="input-text"  v-model="submitData.tareWeight">
                            </span>
                            <span>
                                净重(千克)：
...
import {frontByRFID, afterByRFID, getMeteringByCardNo, meteringGetNumFront, meteringGetNumAfter, meteringopenFront, meteringopenAfter, TrafficLightsFront, TrafficLightsAfter, meteringoInfraRedFront,meteringoInfraRedAfter,meteringoAudio ,getStep , oilGrossWeight , oilTareWeight ,WeightLED} from "@/api/intelligentStorage/public";
...
export default {
  ...
  methods: {
    ...
    getCardNoByAfter() {
      ...
      afterByRFID()
        .then(ret => {
          ...
          if (ret.code === '200') {
            if (ret.data.result=='查询成功！') {
                ...
                vm.getData();
                vm.RFIDstate=2;
                clearTimeout(vm.pageTimer.timer2)
                vm.submitData.startTime=getNowFormatDate();
            }else{
                if (vm.submitData.cardNo=='') {
                  if(vm.RFIDstate == 1){
                ...
                if (vm.submitData.cardNo=='') {
                  vm.pageTimer.timer2=setTimeout(vm.getCardNoByFront,1000);
                  // this.getCardNoByFront();
                }
          }*/
        })
        .catch(err => {
          // layer.closeAll("loading");
          //vm.pageTimer.timer3=setTimeout(vm.getCardNoByFront,1000);
          // this.getCardNoByFront();
          layer.alert(err.message, { closeBtn: 0 });
        });
    },
    getData:function () {
      var vm = this;
      layer.load(2);
      getMeteringByCardNo({cardNo:vm.submitData.cardNo})
        .then(ret => {
          layer.closeAll("loading");
          if (ret.code === '200') {
              console.log(JSON.stringify(ret));
              ...
              getStep({registorNo:ret.data.msg.registorNo})
                .then(ret => {
                  ...
                  if (ret.code === '200') {
                    ...
                    if (ret.data.current_step_name=='皮重') {
    ...
    closeWebSocket:function () {//关闭称重请求
        ...
        if (vm.currentStep=='毛重') {
              ...
              if (vm.currentStep=='皮重') {
                ...
                vm.submitData.incrementDecrementTotal=  add(add(add(except(mul(vm.submitData.netWeight,vm.totalRate),100.0),vm.waterIncrementDecrement),vm.impurityIncrementDecrement),vm.otherIncrementDecrement);//扣量总计   vm.submitData.netWeight*vm.totalRate/100.0
                console.log('------扣量总量：：-------------'+vm.submitData.incrementDecrementTotal+'-----------------')

                vm.submitData.waterIncrementDecrement= except(mul(vm.submitData.netWeight,vm.waterIncrementDecrementRate),100.0); //vm.submitData.netWeight*vm.waterIncrementDecrementRate/100.0;
                vm.submitData.impurityIncrementDecrement= except(mul(vm.submitData.netWeight,vm.impurityIncrementDecrementRate),100.0);  //vm.submitData.netWeight*vm.impurityIncrementDecrementRate/100.0;
                vm.submitData.otherIncrementDecrement= except(mul(vm.submitData.netWeight,vm.otherIncrementDecrementRate),100.0); //vm.submitData.netWeight*vm.otherIncrementDecrementRate/100.0;
  ...
  destroyed (){
      ...
      //clearTimeout(vm.pageTimer.timer1);
      //clearTimeout(vm.pageTimer.timer2);
      clearTimeout(vm.pageTimer.timer3);
      clearTimeout(vm.pageTimer.timer6);
      clearTimeout(vm.pageTimer.timer7);
