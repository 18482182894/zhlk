                ...
                <div class="tag-box-content">
                    <div class="userManage-detail userManage-edit">
                        <div>
                            ...
                            <span>
                                结算单价(元/千克)：
                                <input type="text" class="input-text" v-model="submitData.unitPrice" placeholder="请填写结算单价">
                            </span>
                            <span>
                                结算重量(千克)：
...
<script>
import {oilReimbursementAdd,oilSettlementGetInfo} from "@/api/intelligentStorage/public";
...
export default {
  ...
  methods: {
    getData: function(id) {
      var vm = this;
      layer.load(2);
      oilSettlementGetInfo({cardNo:id,noticeType:1})
        .then(ret => {
          ...
          if (ret.code == 200) {
            ...
            vm.submitData.registerNo = ret.data.detail.registerNo;
          } else {
    ...
    submitEvent: function() {
      var vm = this;
        layer.load(2);
        oilReimbursementAdd(vm.submitData)
          .then(ret => {
            layer.closeAll("loading");
            if (ret.code == 200) {
              layer.msg("结算单提交成功！");
