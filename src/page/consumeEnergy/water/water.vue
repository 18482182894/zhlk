    ...
    <div class="menuManage operationLog ">
      <div class="lookup">
        ...
        <select class="select-style excision" v-model="postData.warehouseGbCode">
          ...
          时间：
          <input type="text" class="input-text Wdate" onclick="WdatePicker({minDate:'%y-%M-%d',dateFmt:'yyyy-MM-dd HH:mm:ss'})" id="startTime">~
          <input type="text" class="input-text Wdate excision" onclick="WdatePicker({minDate:'#F{$dp.$D(\'startTime\')',dateFmt:'yyyy-MM-dd HH:mm:ss'})" id="endTime">
      ...
      <table class="customTable">
        <thead>
          <tr>
            ...
            <th style="width:7.5%;">记水类型</th>
            <th style="width:7.5%;">用水项目</th>
            <th style="width:7.5%;">开始时间</th>
            <th style="width:7.5%;">结算时间</th>
            <th style="width:5%;">运行时间总长(h)</th>
            <th style="width:5%;">作业前水表数据</th>
            <th style="width:5%;">作业后水表数据</th>
            <th style="width:5%;">能耗合计(m³)</th>
        ...
        <tbody>
            <tr v-for="(item,i) in listData">
                ...
                <td>
                    <label v-if="parseInt(item.waterConsumerItem)==1" v-text="'熏蒸作业'"></label>
                    <label v-if="parseInt(item.waterConsumerItem)==2" v-text="'通风作业'"></label>
                    <label v-if="parseInt(item.waterConsumerItem)==3" v-text="'低温储粮'"></label>
                    <label v-if="parseInt(item.waterConsumerItem)==4" v-text="'值仓'"></label>
                    <label v-if="parseInt(item.waterConsumerItem)==5" v-text="'其它'"></label>
                </td>
                ...
                <td>
                        ...
                        <!-- <div class="table-btn btn-submit" v-if="item.recordType=='未完成'" @click="finish(item.id,item.afterJobMeterData);">完成</div> -->
                </td>
            </tr>
...
<script>
import { hydroenergyList, hydroenergyFinish} from "@/api/energyConsumption/public";
...
export default {
  ...
  methods: {
    ...
    finish:function (id,data) {//完成
        var vm = this;
        hydroenergyFinish({id:id,afterJobMeterData:data})
          .then(ret => {
            layer.closeAll("loading");
            if (ret.code == 200) {
              // console.log(ret)
    ...
    jumpDetail: function(id) {
      ...
      var vm = this;
      this.$router.push({
        name: "consumeEnergy_waterDetail",
        query: {
          id: id
        }
      });
